...
@ingroup buffer
*/
uint8_t ModbusMaster::setTransmitBuffer(uint16_t u8Index, uint16_t u16Value)
{
  if (u8Index < ku8MaxBufferSize)
  {
...
{
  uint8_t u8ModbusADU[1024];   //256
  uint16_t u8ModbusADUSize = 0,i, u8Qty;
...
//  uint8_t i, u8Qty;
  ...
  switch(u8MBFunction)
  {
    ...
    case ku8MBWriteMultipleCoils:
      ...
      u8ModbusADU[u8ModbusADUSize++] = highByte(u8Qty);
	  u8ModbusADU[u8ModbusADUSize++] = lowByte(u8Qty);
	  //Serial.print("u8Qty");
	  //Serial.println(u8Qty);
      for (i = 0; i < u8Qty; i++)
      {
        switch(i % 2)
        {
